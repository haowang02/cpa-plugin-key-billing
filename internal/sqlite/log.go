...
func appendLog(tx *sql.Tx, changes billing.Changes) error {
	for _, entry := range changes.Log {
		_, errInsert := tx.Exec(`
			...
			nanos(entry.At), entry.Scope, entry.AuthIndex, entry.UpstreamModel, entry.BillingModel, entry.Failed,
...
func (d *DB) Logs(query billing.LogQuery, since time.Time) (billing.LogView, error) {
	...
	rows, errQuery := d.db.Query(`
		SELECT l.at, l.scope, l.auth_index, l.upstream_model, l.billing_model, l.failed, l.latency_ms, l.ttft_ms,
...
func scanLogRow(rows *sql.Rows) (billing.LogRow, error) {
	...
	if errScan := rows.Scan(&at, &row.Scope, &row.AuthIndex, &row.UpstreamModel, &row.BillingModel, &failed,
